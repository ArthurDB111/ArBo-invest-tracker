...
def _fetch_price(ticker: str) -> Optional[dict]:
    """
    Haal koers op via FMP eerst, dan yfinance als fallback.
    Geeft altijd laatste bekende prijs terug ook buiten beurstijden.
    """
    import httpx, asyncio

    t = ticker.upper().strip()

    # Bouw kandidatenlijst zonder dollar-tekens
    candidates = [t]
    if '.' not in t and '-' not in t and '=' not in t and len(t) <= 6:
        candidates += [t+'.AS', t+'.BR', t+'.DE', t+'.PA', t+'.L']

    # ── 1. FMP via stable/quote ──────────────────────────────────────
    if FMP_KEY:
        try:
    ...
    for sym in candidates:
        try:
            ...
            # Probeer live prijs via fast_info
            try:
                fi    = tk.fast_info
                price = getattr(fi, 'last_price', None)
                prev  = getattr(fi, 'previous_close', None)
                curr  = getattr(fi, 'currency', 'EUR') or 'EUR'
                if price and float(price) > 0:
                    price = round(float(price), 4)
