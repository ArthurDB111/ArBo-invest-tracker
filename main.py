...
async def get_price(ticker: str, user=Depends(verify_token), request: Request = None):
...
@app.post("/prices")
def get_prices(body: dict, user=Depends(verify_token), request: Request = None):
...
# ── Ticker zoeken ────────────────────────────────────────────────────
@app.get("/search")
async def search_ticker(q: str, user=Depends(verify_token), request: Request = None):
