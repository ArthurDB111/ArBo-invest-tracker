...
def _fetch_price(ticker: str) -> Optional[dict]:
    """
    Haal koers op — altijd de laatste bekende prijs, ook buiten beurstijden.
    Volgorde: FMP → yfinance (met auto-suffix)
    """
    t = ticker.upper()
    ...
    # 1. Probeer FMP voor alle kandidaten
    if FMP_KEY:
        ...
        try:
            ...
            result = loop.run_until_complete(try_fmp())
            loop.close()
            if result:
                return result
        except Exception:
    ...
    # 2. Fallback: yfinance — altijd laatste bekende slotkoers
    for sym in candidates:
        try:
            import yfinance as yf
            tk = yf.Ticker(sym)

            # Probeer eerst fast_info (live)
            try:
                info = tk.fast_info
            ...
            # Dagelijkse history (werkt altijd, ook buiten beurstijden)
