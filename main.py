...
async def search_ticker(q: str, user=Depends(verify_token)):
    """
    Zoek naar tickers via naam of symbool.
    Geeft suggesties terug met naam, symbool en beurs.
    """
    import httpx
    ...
    try:
        url = f"https://financialmodelingprep.com/api/v3/search?query={q}&limit=20&apikey={FMP_KEY}"
        ...
        results = r.json()
        # Filter en formatteer resultaten
        # Toon alleen relevante beurzen
        allowed_exchanges = {
            "NASDAQ", "NYSE", "AMEX",           # Amerikaanse beurzen
            "EURONEXT", "ENX",                   # Euronext
            "XETRA",                             # Frankfurt
            "LSE",                               # Londen
            "AMS", "BRU", "PAR",                # Amsterdam, Brussel, Parijs
            "CBOE",                              # ETFs
        }
        ...
        for item in results:
            ...
            stock_exchange = item.get("stockExchange", exchange)

            if sym in seen_symbols:
                continue
            seen_symbols.add(sym)

            # Sla interne/exotische beurzen over
            if exchange and exchange not in allowed_exchanges and not any(
                e in exchange.upper() for e in ["NYSE", "NASDAQ", "EURONEXT", "XETRA", "LSE", "AMS", "BRU"]
            ):
                # Toch toevoegen als het een bekende ticker is
                if not any(c.isdigit() for c in sym) and len(sym) <= 6:
                    pass
                else:
                    continue
            ...
            filtered.append({
                "symbol": sym,
                "name": name,
                "exchange": stock_exchange or exchange,
